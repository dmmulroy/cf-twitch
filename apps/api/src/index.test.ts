 ...
 * cannot be tested with exports.default.fetch. The vi.mock in setup.ts mocks getStub to
 * use cloudflare:test env, but this only applies to direct imports - not to
 * code loaded dynamically via exports.default.fetch. Use DO unit tests for coverage.
 */
describe("Integration Tests", () => {
	...
	// The vi.mock in setup.ts only applies to direct imports, not to code loaded via exports.default.fetch.
	...
	describe.skip("API Routes - Initial State", () => {
		it("should return empty queue initially", async () => {
			const response = await exports.default.fetch("http://localhost/api/queue");
		...
		it("should return no currently playing track initially", async () => {
			const response = await exports.default.fetch("http://localhost/api/now-playing");
		...
		it("should accept limit param for queue", async () => {
			const response = await exports.default.fetch("http://localhost/api/queue?limit=5");
		...
		it("should reject limit above 100", async () => {
			const response = await exports.default.fetch("http://localhost/api/queue?limit=101");
	...
	describe("OAuth Routes", () => {
		it("should have Spotify OAuth callback route", async () => {
			const response = await exports.default.fetch("http://localhost/oauth/spotify/callback");
		...
		it("should have Twitch OAuth callback route", async () => {
			const response = await exports.default.fetch("http://localhost/oauth/twitch/callback");
