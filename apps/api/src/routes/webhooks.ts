...
webhooks.post("/twitch", async (c) => {
	...
	return withLogContext(
		...
		async () => {
			...
			const messageResult = parseEventSubMessage(headers, body);
			...
			const acceptance = await getStub("EVENTSUB_WEBHOOK_DO", messageId).accept({ headers, body });
...
/** Hono router for authenticated and durably accepted Twitch EventSub webhooks. */
